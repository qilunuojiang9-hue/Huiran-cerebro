...
- **不含个人数据**：`cyber_brain.db`（知识库文档、记忆碎片、实体关系、向量索引）已列入 `.gitignore`，克隆后需要自行初始化、自行灌入内容。README 或文档中出现的「598 篇文档 / 5,786 分块 / 6,060 条向量 / 139 条记忆碎片 / 33 个实体」等规模数字，均来自作者**本机实例的实测值**，用于说明系统承载量，**并非仓库自带内容**。
